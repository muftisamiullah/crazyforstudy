...
export default function Subject(){
    ...
    const [title, setTitle] = useState(`Get Reliable ${capitalize(params.subject)} Textbook Solutions Manual`);
    const [description, setDescription] = useState(`Get Access ${capitalize(params.subject)} Textbook Solutions from Crazy For Study. For ${capitalize(params.subject)} textbook answers, ${capitalize(params.subject)} Step-by-step solutions, ${capitalize(params.subject)} Solutions manual and Assignment Help, try Crazy For Study today!`);
    const [keywords, setKeywords] = useState(`${capitalize(params.subject)} textbook solutions, ${capitalize(params.subject)} solutions manual, ${capitalize(params.subject)} textbook solution manuals`);
    ...
    useEffect(()=>{
        if(subSubjects && subSubjects.subject_seo){
            setTitle(subSubjects.subject_seo.textbook_seo_details.meta_title)
            setDescription(subSubjects.subject_seo.textbook_seo_details.meta_description)
